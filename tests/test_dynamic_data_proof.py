...
class DynamicDataProofTests(unittest.TestCase):
    def test_animates_click_edit_synchronized_spreadsheet_and_charts(self):
        ...
        self.assertIn("text='B2'", vf)
        self.assertIn("text='fx'", vf)
        self.assertIn('FILTER(A2', vf)
        self.assertIn("text='A'", vf)
        self.assertIn("text='B'", vf)
        ...
        self.assertIn("text='RECALCULATING...'", vf)
        self.assertIn("text='READY'", vf)
        for row_number in range(2, 7):
            self.assertIn(f"text='{row_number}'", vf)
