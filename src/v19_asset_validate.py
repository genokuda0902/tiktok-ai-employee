...
for key in required:
    candidates=[]
    for ext in ('png','jpg','jpeg','webp','mp4','mov','webm'): candidates += list(ROOT.glob(f'{key}*.{ext}'))
    if not candidates: valid[key]=False; errors.append(f'missing:{key}'); continue
    ...
    except Exception as e: valid[key]=False; errors.append(f'unreadable:{key}:{e}')
ready=not errors
report={'ready':ready,'assets':valid,'errors':errors}
(ROOT/'validation.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
if ready: (ROOT/'hero_ready.json').write_text(json.dumps({'ready':True,'validated':required},ensure_ascii=False,indent=2))
