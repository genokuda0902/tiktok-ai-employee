...
def titlecard(head,sub,mode='robot',accent='#35e7ff'):
 im=bg();d=ImageDraw.Draw(im);text(d,(58,92),'AI時短ラボ',50,'white');d.rectangle((58,158,420,166),fill=accent)
 ...
 text(d,(70,1320),head,72,'white',5,'#020617');text(d,(72,1510),sub,48,accent,3,'#020617');return im
scenes=[('これ、まだ\n手でやってる？','ChatGPT×Excelで一瞬','robot'),('こんな悩み\nありませんか？','繰り返し作業をAIへ','pain'),('ChatGPT×Excel','仕事を一気に時短','apps'),('やりたいことを\n自然な言葉で','入力するだけ','apps'),('数秒で分析','表・要点まで自動','apps'),('結果を確認','必要な情報が一覧に','apps'),('Excelへ貼付','コピペで完成','apps'),('グラフも要点も','一気に見える化','apps'),('Before → After','面倒な作業を短縮','pain'),('他にも使える','要約・メール・可視化','robot'),('保存して試す','毎日1つAI仕事術','robot'),('次回予告','営業メールを10秒で','robot'),('AI時短ラボ','AIで自由な時間を','robot'),('時短 × 効率化','仕事をもっとラクに','robot'),('フォローで続き','明日会社で使えるAI','robot'),('また次の動画で','お会いしましょう！','robot')]
...
for n in range(math.ceil(dur*FPS)):
 t=min(n/FPS,dur-1e-4);idx=next((i for i in range(16) if bounds[i]<=t<bounds[i+1]),15);u=(t-bounds[idx])/max(.01,bounds[idx+1]-bounds[idx]);base=titlecard(*scenes[idx]);z=1+.035*u;rs=base.resize((int(W*z),int(H*z)),Image.Resampling.LANCZOS);l=(rs.width-W)//2;top=(rs.height-H)//2;rs.crop((l,top,l+W,top+H)).save(frames/f'{n:05d}.png',compress_level=1)
...
fc=(f"[1:v]scale=900:1600:flags=lanczos,trim=0:{pd:.3f},setpts=PTS-STARTPTS+{start:.3f}/TB[proof];[0:v][proof]overlay=90:205:enable='between(t,{start:.3f},{end:.3f})':eof_action=pass[v]")
